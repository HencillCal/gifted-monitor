...
router.get('/:id', async (req, res) => {
  try {
    ...
    res.json({ ...monitor, history: await db.getMonitorHistory(monitor.id, 60) });
...
router.post('/', async (req, res) => {
  try {
    ...
    let intervalMins = parseFloat(req.body.intervalMins) || 3;
    ...
    if (intervalMins < config.MIN_PING_INTERVAL_MINS) return res.status(400).json({ error: `Minimum interval is 30 seconds` });
    ...
    if (!req.isAdmin && !req.isSuperAdmin) {
      ...
      if (currentCount >= limit)
        return res.status(403).json({ error: `Monitor limit reached (${limit}). Contact an administrator to increase your limit.` });
    }
...
router.put('/:id', async (req, res) => {
  try {
    ...
    if (method !== undefined) { updates.method = sanitize(method).toUpperCase(); if (updates.method !== 'POST') updates.body = null; }
    if (body !== undefined && (updates.method || monitor.method) === 'POST') updates.body = sanitize(body) || null;
    if (intervalMins !== undefined) {
      const i = parseFloat(intervalMins);
      if (i < config.MIN_PING_INTERVAL_MINS) return res.status(400).json({ error: 'Minimum interval is 30 seconds' });
