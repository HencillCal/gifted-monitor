async function createMysqlAdapter(DATABASE_URL) {
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS users (
    ...
    email VARCHAR(150) UNIQUE NOT NULL, whatsapp VARCHAR(20) NOT NULL, password_hash TEXT NOT NULL,
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS otp_codes (
    id INT AUTO_INCREMENT PRIMARY KEY, email VARCHAR(150) NOT NULL, code VARCHAR(10) NOT NULL,
  ...
  for (const col of ['is_superadmin TINYINT(1) DEFAULT 0','is_disabled TINYINT(1) DEFAULT 0','avatar LONGTEXT','notify_down TINYINT(1) DEFAULT 1','notify_up TINYINT(1) DEFAULT 1','monitor_limit INT DEFAULT 20'])
  ...
  return {
    ...
    async getAllActiveMonitors() { const [r]=await pool.query(`SELECT m.*,u.name as user_name,u.whatsapp as user_whatsapp FROM monitors m JOIN users u ON m.user_id=u.id WHERE m.is_active=1`); return r; },
